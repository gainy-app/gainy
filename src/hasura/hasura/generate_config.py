        ...
        for key in ['array_relationships', 'object_relationships']:
            ...
            for i in table[key]:
                if 'manual_configuration' not in i['using']:
                    continue
                if not should_replace_schema(i['using']['manual_configuration']
                                             ['remote_table']['schema']):
                    continue
                i['using']['manual_configuration']['remote_table'][
                    'schema'] = HASURA_GRAPHQL_PUBLIC_SCHEMA_NAME
...
def generate_actions():
    ...
    for object_config in actions['custom_types']['objects']:
        ...
        for relationship in object_config['relationships']:
            if 'remote_table' not in relationship or 'schema' not in relationship[
                    'remote_table'] or not should_replace_schema(
