         ...
         (
             select distinct on (
                 profile_portfolio_transactions.id
                   ...
                   historical_prices_aggregated.time::timestamp as updated_at,
                   sign(profile_portfolio_transactions.quantity)::numeric * (
                               ...
                               over (partition by historical_prices_aggregated.symbol ORDER BY historical_prices_aggregated.time RANGE INTERVAL '1 day' PRECEDING) -
                               1
                       )                                        as relative_gain_1d,
                   sign(profile_portfolio_transactions.quantity)::numeric * (
                               historical_prices_aggregated.adjusted_close::numeric /
                               first_value(historical_prices_aggregated.adjusted_close::numeric)
                               over (partition by historical_prices_aggregated.symbol ORDER BY historical_prices_aggregated.time RANGE INTERVAL '1 week' PRECEDING) -
                               1
                       )                                        as relative_gain_1w,
                   sign(profile_portfolio_transactions.quantity)::numeric * (
                               historical_prices_aggregated.adjusted_close::numeric /
                               first_value(historical_prices_aggregated.adjusted_close::numeric)
                               over (partition by historical_prices_aggregated.symbol ORDER BY historical_prices_aggregated.time RANGE INTERVAL '1 month' PRECEDING) -
                               1
                       )                                        as relative_gain_1m,
                   sign(profile_portfolio_transactions.quantity)::numeric * (
                               historical_prices_aggregated.adjusted_close::numeric /
                               first_value(historical_prices_aggregated.adjusted_close::numeric)
                               over (partition by historical_prices_aggregated.symbol ORDER BY historical_prices_aggregated.time RANGE INTERVAL '3 months' PRECEDING) -
                               1
                       )                                        as relative_gain_3m,
                   sign(profile_portfolio_transactions.quantity)::numeric * (
                               historical_prices_aggregated.adjusted_close::numeric /
                               first_value(historical_prices_aggregated.adjusted_close::numeric)
                               over (partition by historical_prices_aggregated.symbol ORDER BY historical_prices_aggregated.time RANGE INTERVAL '1 year' PRECEDING) -
                               1
                       )                                        as relative_gain_1y,
                   sign(profile_portfolio_transactions.quantity)::numeric * (
                               historical_prices_aggregated.adjusted_close::numeric /
                               first_value(historical_prices_aggregated.adjusted_close::numeric)
                               over (partition by historical_prices_aggregated.symbol ORDER BY historical_prices_aggregated.time RANGE INTERVAL '5 years' PRECEDING) -
                               1
                       )                                        as relative_gain_5y,
                   sign(profile_portfolio_transactions.quantity)::numeric * (
                               historical_prices_aggregated.adjusted_close::numeric /
                               first_value(historical_prices_aggregated.adjusted_close::numeric)
                               over (partition by historical_prices_aggregated.symbol ORDER BY historical_prices_aggregated.time RANGE UNBOUNDED PRECEDING) -
                               1
