...
UNION
select interests.id as interest_id, ct.ticker_code as symbol from ct join interests on interests.name = 'Hotels and Cruise Lines' where ct.g_industry ilike '%hotel%' and ct.g_industry not ilike '%reit%' or ct.g_industry ilike '%onlin%travel%compan%' or ct.g_industry ilike '%maritime%transport%'
UNION
