...
def create_downstream_operators(dag):
    ...
    data_check = BashOperator(
        ...
        bash_command=
        f'[ $(PGPASSWORD=$PG_PASSWORD psql -q --csv -h $PG_ADDRESS -p $PG_PORT -U $PG_USERNAME $PG_DATABASE -c "set search_path = $DBT_TARGET_SCHEMA" -f "{MELTANO_PROJECT_ROOT}/scripts/data_checks/autogenerated_industries.sql" | wc -l) == 1 ]',
        dag=dag)
