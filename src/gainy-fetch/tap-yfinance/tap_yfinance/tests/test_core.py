...
@pytest.mark.vcr
def test_selected():
    tap1 = Tapyfinance(config=SAMPLE_CONFIG, parse_env_config=True)
...
def test_partitions_and_state():
    ...
    tap1 = Tapyfinance(config=config, parse_env_config=True)

    assert tap1.streams['fundamentals'].partitions == [{'Code': 'AAPL'}, {'Code': 'F'}, {'Code': 'IBM'}]

    tap1 = Tapyfinance(config=config, state=state, parse_env_config=True)
