...
(
    ...
    select (tds.code || '_' || tds.date || '_1d')::varchar as id,
           tds.code                                        as symbol,
           tds.date::timestamp                             as time,
           ...
           coalesce(hp.adjusted_close,
                    linear_interpolate(
                            extract(epoch from tds."date"),
                            LAST_VALUE_IGNORENULLS(extract(epoch from hp."date")) over (lookback),
                            LAST_VALUE_IGNORENULLS(hp.adjusted_close) over (lookback),
                            LAST_VALUE_IGNORENULLS(extract(epoch from hp."date")) over (lookforward),
                            LAST_VALUE_IGNORENULLS(hp.adjusted_close) over (lookforward)
                        ),
                    LAST_VALUE_IGNORENULLS(hp.adjusted_close) over (lookforward),
                    LAST_VALUE_IGNORENULLS(hp.adjusted_close) over (lookback),
                    0)                                     as adjusted_close,
    ...
    from tickers_dates_skeleton tds
        ...
        window
            ...
            lookforward as (partition by tds.code order by tds."date" desc)
)
