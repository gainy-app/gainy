...
select t.symbol || '_' || t.date_month as id,
       t.symbol                        as symbol,
...
from (
         ...
         join {{ ref('historical_prices') }} hp_open on hp_open.symbol = t.symbol and hp_open.date = t.open_date
         join {{ ref('historical_prices') }} hp_close on hp_close.symbol = t.symbol and hp_close.date = t.close_date
{% if is_incremental() %}
         left join {{ this }} old_data on old_data.symbol = t.symbol and old_data.datetime = t.date_month
...
--   Hash Cond: (((hp_open.symbol)::text = (hp_close.symbol)::text) AND (t.close_date = hp_close.date))
--   ->  Hash Join  (cost=1857253.06..6910861.97 rows=1697250 width=54) (actual time=19067.470..74758.818 rows=1914573 loops=1)
--         Hash Cond: (((t.symbol)::text = (hp_open.symbol)::text) AND (t.open_date = hp_open.date))
...
--                     Group Key: historical_prices.symbol, historical_prices.date_month
--                     ->  Index Scan using symbol__date_month__date on historical_prices  (cost=0.56..3939472.16 rows=39589540 width=41) (actual time=0.012..23662.185 rows=39580763 loops=1)
