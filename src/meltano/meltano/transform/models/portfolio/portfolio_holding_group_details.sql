...
select profile_holdings_normalized_all.holding_group_id,
       ...
       collection_id, -- deprecated
       min(portfolio_holding_details.purchase_date)                as purchase_date,
       min(portfolio_holding_group_gains.relative_gain_total)      as relative_gain_total, -- deprecated
       min(portfolio_holding_group_gains.relative_gain_1d)         as relative_gain_1d, -- deprecated
       min(portfolio_holding_group_gains.value_to_portfolio_value) as value_to_portfolio_value, -- deprecated
...
group by profile_holdings_normalized_all.holding_group_id, profile_holdings_normalized_all.profile_id, collection_id
...
from {{ ref('portfolio_holding_details') }}
         ...
         join {{ ref('portfolio_securities_normalized') }}
              on portfolio_securities_normalized.id = profile_holdings_normalized_all.security_id
         join {{ ref('portfolio_holding_group_gains') }}
              on portfolio_holding_group_gains.profile_id = profile_holdings_normalized_all.profile_id
...
where not profile_holdings_normalized_all.is_hidden
group by profile_holdings_normalized_all.holding_group_id, profile_holdings_normalized_all.profile_id, portfolio_securities_normalized.ticker_symbol
