...
with data as
    (
        ...
        select t.*,
               holding_since.date::timestamp as holding_since
        from (
                 select case
                 ...
                 select distinct on (
                     profile_id, security_id, account_id
                     ) case
                           when portfolio_securities_normalized.type = 'cash'
                       ...
                       '_plaid_' || security_id || '_' || account_id                     as holding_id_v2,
                 ...
                 left join (
                    ...
                    ) holding_since using (profile_id)
