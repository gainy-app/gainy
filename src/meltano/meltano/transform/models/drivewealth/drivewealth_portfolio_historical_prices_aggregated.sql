{{
  config(
    ...
    unique_key = "id",
    post_hook=[
      pk('profile_id, period, collection_id, symbol, datetime'),
      index('id', true),
    ]
  )
}}
...
with chart_1w as
         (
             ...
             group by profile_id, collection_id, symbol, date_week
     ),
     ...
     chart_1m as
         (
             select profile_id,
                    ...
                    max(updated_at) as updated_at
             from {{ ref('drivewealth_portfolio_historical_holdings') }}
             group by profile_id, collection_id, symbol, date_month
     ),

     data as
         (
             select drivewealth_portfolio_historical_holdings.profile_id,
                    drivewealth_portfolio_historical_holdings.collection_id,
             ...
             group by drivewealth_portfolio_historical_holdings.profile_id, drivewealth_portfolio_historical_holdings.collection_id,
             ...
             from chart_1w data
                      join {{ ref('drivewealth_portfolio_historical_holdings') }} dhh_open
                           on dhh_open.profile_id = data.profile_id
                               and dhh_open.collection_id = data.collection_id
                      ...
                      join {{ ref('drivewealth_portfolio_historical_holdings') }} dhh_close
                           on dhh_close.profile_id = data.profile_id
                               and dhh_close.collection_id = data.collection_id
...
select data.*,
       'ttf_dw_' || profile_id || '_' || collection_id || '_' || symbol                        as transaction_uniq_id,
       profile_id || '_' || period || '_' || collection_id || '_' || symbol || '_' || datetime as id
from data

{% if is_incremental() %}
         left join {{ this }} old_data using (profile_id, period, collection_id, symbol, datetime)
