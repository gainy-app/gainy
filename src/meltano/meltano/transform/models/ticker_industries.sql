...
-- manual industries, don't touch it
     ticker_manual_industries as (
         ...
         from {{ source('gainy', 'gainy_ticker_industries') }} rgti -- 1 row
                  ...
                  join ticker_auto_industries ai on ai.symbol = rgti.code
         group by rgti.code, gi.id::int
     ),
...
-- union 3 parts
     ticker_industries_union as (
         ...
         select u3.symbol,
                industry_id, -- the more company diversified - the more it's recognizible by the people (and that approach really correlates with ranking from manual industries)
                (sim * (1e-30 + avg(sim) over (partition by u3.symbol))) as sim,
                industry_preorder
         from union3 u3
     ),
     ...
     ticker_industries_nrmlzd as (
         select tiu.symbol,
                tiu.industry_id,
                (tiu.sim - (min(tiu.sim) over (partition by tiu.industry_id) - 1e-1)) -- inside each industry-"world" normalization (companies-involvement-wise)
                    / (1e-30 +
                       ((1e-1 + max(tiu.sim) over (partition by tiu.industry_id)) -- (we need this either for measure entrance into interest by company involvement in "world" of industry (which is assigned to interest) )
                           - (min(tiu.sim) over (partition by tiu.industry_id) - 1e-1))
                    ) as sim,
...
from ticker_industries_nrmlzd tin
         join common_stocks cs on cs.symbol = tin.symbol
