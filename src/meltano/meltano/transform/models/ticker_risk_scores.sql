             ...
             with ticker_volatility_nrmlzd as
                      (
                          ...
                          from {{ ref('ticker_metrics') }} tm
                                   left join (select max(absolute_historical_volatility_adjusted_current) as v_max,
                                       min(absolute_historical_volatility_adjusted_current) as v_min
                                from {{ ref('ticker_metrics') }} tm
                                where tm.absolute_historical_volatility_adjusted_current is not null) as maxmin
                          on true
