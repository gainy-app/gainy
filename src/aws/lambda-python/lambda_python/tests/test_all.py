...
from _common import get_action_event, get_trigger_event, PROFILE_ID, USER_ID

# Actions
def test_get_recommended_collections():
    event = get_action_event("get_recommended_collections", {"profile_id": PROFILE_ID}, USER_ID)
...
def test_create_plaid_link_token():
    event = get_action_event("create_plaid_link_token", {"profile_id": PROFILE_ID, "redirect_uri": "https://app.gainy.application.ios", "env": "sandbox"}, USER_ID)
    response = action_dispatcher.handle(event)
    assert "code" not in response
...
def test_get_portfolio_chart():
    event = get_action_event("get_portfolio_chart", {"profile_id": PROFILE_ID}, USER_ID)
    response = action_dispatcher.handle(event)
    assert "code" not in response
    assert isinstance(response, list)
    assert len(response)
    assert set({'period', 'datetime', 'open', 'high', 'low', 'close', 'adjusted_close'}).issubset(set(response[0].keys()))

# Triggers
def test_set_user_categories():
    event = get_trigger_event("set_user_categories", "insert", {
            "old": {},
            "new": {
...
def test_set_recommendations():
    for trigger_name in ['recommendations__profile_categories', 'recommendations__profile_interests']:
