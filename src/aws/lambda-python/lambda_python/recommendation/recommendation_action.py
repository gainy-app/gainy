...
from recommendation.data_access import read_profile_industry_vector, \
    ...
    read_categories_risks, read_collection_tickers, read_collection_match_scores
...
class GetRecommendedCollections(HasuraAction):
    ...
    def apply(self, db_conn, input_params, headers):
        ...
        collection_match_scores = read_collection_match_scores(db_conn, profile_id)

        ranked_collections = self.ranking.rank(profile_v,
                                               collection_vs,
                                               df=document_frequencies,
                                               size=corpus_size,
                                               collection_match_scores=collection_match_scores
                                               )
