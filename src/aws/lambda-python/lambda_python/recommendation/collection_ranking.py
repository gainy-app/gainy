...
class CollectionRanking(ABC):
    ...
    def rank(self, profile_v: DimVector, collection_vs: list[DimVector], **params) -> list[RankedItem]:
        ranked_items = list(map(
            lambda collection_v: RankedItem(collection_v, self.rank_score(profile_v, collection_v, **params)),
            collection_vs)
        )
    ...
    @abstractmethod
    def rank_score(self, profile_v: DimVector, collection_v: DimVector, **params) -> float:
        pass
...
class TFIDFWithNorm1_5CollectionRanking(CollectionRanking):
    ...
    def rank_score(self, profile_v: DimVector, collection_v: DimVector, **params) -> float:
        ...
        norm_collection_v = self._normalize_collection_vector(collection_v, df, corpus_size)

        return self._similarity(norm_profile_v, norm_collection_v)

    @staticmethod
    def _similarity(norm_profile_v, norm_collection_v):
        return norm_profile_v.cosine_similarity(norm_collection_v, norm_order=1.5)

    @staticmethod
    def _normalize_profile_vector(profile_v: DimVector) -> DimVector:
        new_coordinates = {}
        for (dim, value) in zip(profile_v.dims, double_normalization_k(profile_v).values):
            tf = value
            idf = 1
    ...
    @staticmethod
    def _normalize_collection_vector(collection_v: DimVector, df, corpus_size) -> DimVector:
        new_coordinates = {}
