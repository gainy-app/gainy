...
class HasuraDispatcher(ABC):
    ...
    def handle(self, event, context):
        ...
        with psycopg2.connect(self.db_conn_string) as db_conn:
            try:
                ...
                traceback.print_exc()
                return self.format_response(he.http_code, {"message": he.message, "code": he.http_code})
            except Exception as e:
                traceback.print_exc()
                return self.format_response(500, {"message": str(e), "code": 500})
    ...
    def choose_function_by_name(self, function_name):
        filtered_actions = list(filter(lambda function: function_name == function.name, self.functions))
        if len(filtered_actions) != 1:
            raise HasuraActionException(400, f"`{function_name}` is not a valid action or trigger")
    ...
    def format_response(self, http_code, response_body):
        return base_response(http_code, response_body) if self.is_gateway_proxy else response_body
    ...
    def check_authorization(self, db_conn, profile_id, session_variables):
        try:
            ...
            traceback.print_exc()
            raise HasuraActionException(401, f"Unauthorized access to profile `{profile_id}`")
...
class HasuraTriggerDispatcher(HasuraDispatcher):
    ...
    def apply(self, db_conn, request):
        ...
        print(f"Start trigger: {trigger.name}")
