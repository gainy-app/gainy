...
class PortfolioService:
    ...
    def get_holdings(self, db_conn, profile_id):
        ...
        for access_token in self.__get_access_tokens(db_conn, profile_id):
            token_data = self.__get_service(access_token['service']).get_holdings(db_conn, access_token)

            holdings += token_data['holdings']
            securities += token_data['securities']
            accounts += token_data['accounts']

        self.persist_holding_data(db_conn, profile_id, securities, accounts, holdings)

        return holdings

    def sync_token_holdings(self, db_conn, access_token):
        data = self.__get_service(access_token['service']).get_holdings(db_conn, access_token)
        holdings = data['holdings']
        self.persist_holding_data(db_conn, access_token['profile_id'], data['securities'], data['accounts'], holdings)
    ...
    def get_transactions(self, db_conn, profile_id, count=100, offset=0):
        ...
        for access_token in self.__get_access_tokens(db_conn, profile_id):
            token_data = self.__get_service(access_token['service']).get_transactions(db_conn,
                                                                                      plaid_access_token,
                                                                                      count=count,
                                                                                      offset=offset)
        ...
        self.persist_transaction_data(db_conn, profile_id, securities, accounts, transactions)

        return transactions

    def sync_token_transactions(self, db_conn, access_token):
        transactions_count = 0
        count = self.__get_service(access_token['service']).max_transactions_limit()
        for offset in range(0, 1000000, count):
            data = self.__get_service(access_token['service']).get_transactions(db_conn, access_token, count=count, offset=offset)
            transactions = data['transactions']
            self.persist_transaction_data(db_conn, access_token['profile_id'], data['securities'], data['accounts'], transactions)
    ...
    def persist_holding_data(self, db_conn, profile_id, securities, accounts, holdings):
    ...
    def persist_transaction_data(self, db_conn, profile_id, securities, accounts, transactions):
        securities_dict = self.__persist_securities(db_conn, securities)
        accounts_dict = self.__persist_accounts(db_conn, accounts, profile_id)
    ...
    def __get_access_tokens(self, db_conn, profile_id):
        with db_conn.cursor() as cursor:
            ...
            return [
                dict(zip(['id', 'access_token', 'service'], row + (SERVICE_PLAID, )))
                for row in access_tokens
            ]
