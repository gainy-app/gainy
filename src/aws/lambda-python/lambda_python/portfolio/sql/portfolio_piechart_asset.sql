         ...
         (
             select holdings.profile_id,
                    ...
                    sum(portfolio_holding_group_gains.relative_gain_1d *
                        portfolio_holding_gains.actual_value) as relative_daily_change,
                    sum(portfolio_holding_group_gains.absolute_gain_1d *
                        portfolio_holding_gains.actual_value) as absolute_daily_change,
                    sum(coalesce(case
                                     when type = 'cash'
                                         then pending_cash
                                     end, 0) +
             ...
             from holdings
                      left join portfolio_holding_gains using (profile_id, holding_id_v2)
...
select portfolio_assets.profile_id,
       ...
       entity_id,
       entity_name,
       coalesce(absolute_daily_change / weight, 0) as absolute_daily_change,
       coalesce(relative_daily_change / weight, 0) as relative_daily_change,
       absolute_value
from portfolio_assets
