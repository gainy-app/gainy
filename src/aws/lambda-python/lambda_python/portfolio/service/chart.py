...
class PortfolioChartService:
    ...
    def get_portfolio_chart(self, db_conn, profile_id, filter):
        ...
        self._filter_query_by_profile_id(params, where_clause, join_clause, profile_id)
        self._filter_query_by_periods(params, where_clause, join_clause, filter)
        self._filter_query_by_account_ids(params, where_clause, join_clause, filter)
        self._filter_query_by_institution_ids(params, where_clause, join_clause, filter)
        self._filter_query_by_access_token_ids(params, where_clause, join_clause, filter)
        self._filter_query_by_interest_ids(params, where_clause, join_clause, filter)
        self._filter_query_by_category_ids(params, where_clause, join_clause, filter)
        self._filter_query_by_security_types(params, where_clause, join_clause, filter)
        self._filter_query_by_ltt_only(params, where_clause, join_clause, filter)

        rows = self._execute_query(params, where_clause, join_clause, query, db_conn)

        rows = list(self._filter_chart_by_transaction_count(rows))
        if not rows:
            return []

        static_value = self._get_chart_static_value(db_conn, profile_id, filter)
        if static_value:
            for i in rows:
                i['open'] += static_value
                i['high'] += static_value
    ...
    def get_portfolio_chart_previous_period_close(self, db_conn, profile_id, filter):
        with open(os.path.join(SCRIPT_DIR, "../sql/portfolio_chart_prev_close.sql")) as f:
            query = f.read()
        ...
        data = self._execute_query(params, where_clause, join_clause, query, db_conn)
    ...
    def _get_chart_static_value(self, db_conn, profile_id, filter):
        ...
        static_data = self._execute_query(params, where_clause, join_clause, query, db_conn)
    ...
    def _should_return_empty_result(self, filter):
        ...
        if filter.account_ids is not None and not len(filter.account_ids):
            return True
        if filter.institution_ids is not None and not len(filter.institution_ids):
            return True
        if filter.access_token_ids is not None and not len(filter.access_token_ids):
            return True
        ...
        if filter.category_ids is not None and not len(filter.category_ids):
            return True
        if filter.security_types is not None and not len(filter.security_types):
            return True

        return False

    def _filter_query_by_profile_id(self, params, where_clause, join_clause, profile_id):
        params["profile_id"] = profile_id,
        where_clause.append(sql.SQL("profile_plaid_access_tokens.profile_id = %(profile_id)s"))

    def _filter_query_by_periods(self, params, where_clause, join_clause, filter):
        if not filter.periods:
            return

        where_clause.append(sql.SQL("portfolio_transaction_chart.period in %(periods)s"))
        params['periods'] = tuple(filter.periods)

    def _filter_query_by_account_ids(self, params, where_clause, join_clause, filter):
        if not filter.account_ids:
            return

        where_clause.append(
            sql.SQL(
                "profile_portfolio_accounts.id in %(account_ids)s"
            ))
        params['account_ids'] = tuple(filter.account_ids)

    def _filter_query_by_institution_ids(self, params, where_clause, join_clause, filter):
    ...
    def _filter_query_by_access_token_ids(self, params, where_clause, join_clause, filter):
    ...
    def _filter_query_by_interest_ids(self, params, where_clause, join_clause, filter):
    ...
    def _filter_query_by_category_ids(self, params, where_clause, join_clause, filter):
    ...
    def _filter_query_by_security_types(self, params, where_clause, join_clause, filter):
    ...
    def _filter_query_by_ltt_only(self, params, where_clause, join_clause, filter):
    ...
    def _execute_query(self, params, where_clause, join_clause, query, db_conn):
        join_clause = sql.SQL("\n").join(join_clause)
        where_clause = sql.SQL(' and ').join(where_clause)
        query = sql.SQL(query).format(where_clause=where_clause, join_clause=join_clause)
