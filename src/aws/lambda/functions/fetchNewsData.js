...
exports.fetchNewsData = async (event) => {
  ...
  const result = (await gnewsFetch(['search'], {
    q: symbol,
    max: limit || defaultLimit,
  }, cacheTTL)).articles.map((row) => {
    const {
      publishedAt, title, description, url, image, source
    } = row;
    const datetime = moment(publishedAt).format();
    return {
      datetime, title, description, url, imageUrl: image, sourceName: source.name, sourceUrl:source.url
    };
  });
