...
const Header = () => {
  ...
  const [requestAccessToken, setRequestAccessToken] = useState(false);
  const handleRequestAccessTokenChange = () => {
    setRequestAccessToken(!requestAccessToken);
  };
      ...
      {!linkSuccess ? (
        <>
          ...
          linkToken == null && backend ? (
            ...
            <div className={styles.linkButton}>
              <p className={styles.introPar}>
                <input type="checkbox"
                       checked={requestAccessToken}
                       onChange={handleRequestAccessTokenChange}/>
                Exchange public_token for access_token
              </p>
              <Link requestAccessToken={requestAccessToken}/>
            </div>
          )}
        </>
      ) : (
        <>
          ...
          <div className={styles.itemAccessContainer}>
            ...
            <p className={styles.itemAccessRow}>
              <span className={styles.idName}>public_token</span>
              <span className={styles.tokenText}>{publicToken}</span>
            </p>
