...
PGPASSWORD="${pg_password}" psql -h "${pg_host}" -p "${pg_port}" -U "${pg_username}" "${pg_dbname}" -P pager -c "REVOKE SELECT ON ALL TABLES IN SCHEMA airflow FROM datadog; REVOKE USAGE ON SCHEMA airflow FROM datadog; drop user if exists datadog; create user datadog with password '${pg_datadog_password}'; grant pg_monitor to datadog; grant SELECT ON pg_stat_database to datadog; GRANT USAGE ON SCHEMA airflow TO datadog;GRANT SELECT ON ALL TABLES IN SCHEMA airflow TO datadog;"
