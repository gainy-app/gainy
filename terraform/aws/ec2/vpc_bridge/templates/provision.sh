...
PGPASSWORD="${PG_PASSWORD}" psql -h "${PG_HOST}" -p "${PG_PORT}" -U "${PG_USERNAME}" "${PG_DBNAME}" -P pager -c "create table ${PUBLIC_SCHEMA_NAME}.data_checks (id varchar not null constraint pk_data_checks primary key, symbol varchar, code varchar, period varchar, message text, updated_at timestamp with time zone);"
PGPASSWORD="${PG_PASSWORD}" psql -h "${PG_HOST}" -p "${PG_PORT}" -U "${PG_USERNAME}" "${PG_DBNAME}" -P pager -c "GRANT USAGE ON SCHEMA ${PUBLIC_SCHEMA_NAME} TO datadog; GRANT SELECT ON ${PUBLIC_SCHEMA_NAME}.data_checks TO datadog;"
