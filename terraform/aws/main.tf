...
module "ecs-service" {
  ...
  pg_username                 = module.rds.db_instance.username
  pg_dbname                   = module.rds.db_instance.name
  pg_replica_uris             = sensitive(join(",", [for index, replica in module.rds.db_replica[*] : format("postgres://%s:%s@%s:%d/%s", replica.username, nonsensitive(replica.password), replica.address, replica.port, replica.name)]))
