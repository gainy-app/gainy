...
module "ecs-service" {
  ...
  pg_replica_uris         = join(",", [for index, replica in module.rds.db_replica[*] : format("postgres://%s:%s@%s:%d/%s", replica.username, replica.password, replica.address, replica.port, replica.name)])
