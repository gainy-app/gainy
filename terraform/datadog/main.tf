...
resource "datadog_monitor" "api_gateway_request_count" {
...
resource "datadog_monitor" "api_gateway_latency" {
  ...
  type    = "query alert"
  message = "API Gateway Latency Monitor triggered. Notify: @slack-${var.slack_channel_name} <!channel>"
  #  escalation_message = "Escalation message @pagerduty"

  query = "avg(last_1d):anomalies(sum:aws.apigateway.latency{stage:*_production}.as_count(), 'basic', 2, direction='above', alert_window='last_6h', interval=300, count_default_zero='true') > 1"
...
resource "datadog_monitor" "api_gateway_errors" {
